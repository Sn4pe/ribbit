// Generate public/og.svg from the ribbit engine itself. Run: bun run scripts/gen-og.mjs
import { writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { toSVG } from "ribbit";

const field = toSVG("null-frog", { preset: "og", pattern: "wave" });
// Inject the wordmark over the generated field, kept clear of every edge.
...
const out = field.replace("</svg>", `${overlay}</svg>`);
